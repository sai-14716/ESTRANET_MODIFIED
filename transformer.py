...
def shape_list(x):
    ...
    dynamic = tf.shape(x)
    # Reverted to original, correct logic:
...
class PositionalFeature(tf.keras.layers.Layer):
    ...
    def call(self, slen, bsz=None):
        ...
        # Fixed: self.slopes (plural)
        normalized_slopes = (1. / tf.cast(slen-1, tf.float32)) * self.slopes
        ...
        # Fixed: tf.cast(slen-1, ...)
        pos_feature_slopes = tf.cast(slen-1, tf.float32)*tf.reshape(pos_feature_slopes, [1, -1])
...
class SoftmaxAttention(tf.keras.layers.Layer):
    ...
    def call(self, inp, softmax_attn_smoothing, training=False):
        ...
        shape_prefix = tf.shape(attn_out)[:2]  # [batch, seq_len]
        new_shape = tf.concat([shape_prefix, tf.constant([-1], dtype=tf.int32)], axis=0)
        attn_out = tf.reshape(attn_out, new_shape)
...
class Transformer(tf.keras.Model):
    ...
    def call(self, inp, softmax_attn_smoothing=1, training=False):
        ...
        if self.softmax_attn:
            core_out, softmax_attn_score = self.out_attn(inp=core_out, softmax_attn_smoothing=softmax_attn_smoothing, training=training)
        else:
