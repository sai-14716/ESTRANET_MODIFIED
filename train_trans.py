    ...
    def train_steps(iterator, steps, bsz, global_step):
        ...
        def step_fn(inps, lbls, global_step):
            ...
            with tf.GradientTape() as tape:
                ...
                logits = model(inps, softmax_attn_smoothing=softmax_attn_smoothing, training=True)[0]
                if FLAGS.dataset == 'CHES20':
...
def evaluate(data, strategy, chk_name):
    ...
    with strategy.scope():
        if FLAGS.dataset == 'CHES20':
            ...
            model = create_model(256)
# We don't need the optimizer for evaluation, so we don't create it
        # optimizer = tf.keras.optimizers.Adam(learning_rate=FLAGS.learning_rate) 
        
        # Create a checkpoint object that *only* tracks the model
        ...
        try:
            ...
            tf.compat.v1.logging.error(f"Could not restore checkpoint. Error: {e}")
            return None
...
def main(unused_argv):
    ...
    if FLAGS.dataset == 'ASCAD':
        train_data = data_utils.Dataset(data_path=FLAGS.data_path, split="train", 
                input_length=FLAGS.input_length, data_desync=FLAGS.data_desync, max_samples = 200)
        test_data = data_utils.Dataset(data_path=FLAGS.data_path, split="test",
                input_length=FLAGS.input_length, data_desync=FLAGS.data_desync, max_samples = 50)
